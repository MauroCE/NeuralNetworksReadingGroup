---
title: "Reading Group Schedule"
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

The current schedule for the Neural Network reading group is


```{r, echo=FALSE}
# All available members
members <- c(
  "Mauro Camara Escudero", 
  "Alexander Modell", 
  "Andi Wang", 
  "Anthony Lee", 
  "Chang Zhang", 
  "Christophe Andrieu",
  "Henry Reeve", 
  "Mark Beaumont", 
  "Mathieu Gerber", 
  "Mingxuan Yi", 
  "Nick Whiteley", 
  "Patrick Rubin-Delanchy", 
  "Pierre-Aurelien Gilliot", 
  "Robert Allison",
  "Sam Tickle", 
  "Song Liu"
)
# Date of Presentation
dates_tb1 <- seq(as.Date("2020-10-13"), as.Date("2020-12-15"), by="weeks")
dates_tb2 <- seq(as.Date("2020-02-02"), as.Date("2020-03-09"), by="weeks")
dates     <- c(dates_tb1, dates_tb2)
# Order of Presentations
index     <- c(5, 7, 3, 6, 2, 11, 4, 16, 8, 9, 1, 13, 10, 15, 12, 14)
names <- members[index]
# ZOOM during COVID
location <- rep("ZOOM", 16)
schedule <- data.frame(Date=dates,Name=names,Room=location)
knitr::kable(schedule)
```



<!-- <style> -->
<!-- table, th, td { -->
<!--   border: 1px solid black; -->
<!--   border-collapse: collapse; -->
<!-- } -->
<!-- th, td { -->
<!--   padding: 15px; -->
<!-- } -->
<!-- </style> -->

<!-- <table> -->
<!--   <tr> -->
<!--     <th>Date</th> -->
<!--     <th>Name</th> -->
<!--   </tr> -->
<!--   <tr> -->
<!--     <td>17th December</td> -->
<!--     <td>Anthony Lee</td> -->
<!--   </tr> -->
<!--   <tr> -->
<!--     <td>14th January</td> -->
<!--     <td>Song Liu</td> -->
<!--   </tr> -->
<!--   <tr> -->
<!--     <td>21st January</td> -->
<!--     <td>Mark Beaumont</td> -->
<!--   </tr> -->
<!--   <tr> -->
<!--     <td>28th January</td> -->
<!--     <td>Mauro Camara Escudero</td> -->
<!--   </tr> -->
<!--   <tr> -->
<!--     <td>4th February</td> -->
<!--     <td>Patrick Rubin-Delanchy</td> -->
<!--   </tr> -->
<!--   <tr> -->
<!--     <td>11th February</td> -->
<!--     <td>Andi Wang</td> -->
<!--   </tr> -->
<!--   <tr> -->
<!--     <td>18th February</td> -->
<!--     <td>Mathieu Gerber</td> -->
<!--   </tr> -->
<!--   <tr> -->
<!--     <td>25th February</td> -->
<!--     <td>Nick Whiteley</td> -->
<!--   </tr> -->
<!-- </table> -->


